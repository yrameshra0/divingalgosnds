module.exports = function newLinkedList(elems) {
  let entryNode = undefined;
  let kThElem = undefined;
  ...
  function recurKElem(k) {
    (function kThFromLast(k, traverseNode) {
      if (traverseNode === undefined) return 0;

      let currVal = kThFromLast(k, traverseNode.next) + 1;

      if (currVal === k) kThElem = createNode(traverseNode.data);

      return currVal;
    })(k, entryNode);
  }

  function iterateKElem(k) {
    let pointer1 = entryNode;
    let pointer2 = entryNode;
